...
def main(args):
    ...
    log_path = f'logs/{args.name}_{mode_name}_{args.run_name}'
...
#######################################################
    ...
    val_dataloader = DataLoader(val_dataset, batch_size=args.batch_size, shuffle=False, num_workers=16,pin_memory=True)
...
#checkpoint_D=checkpoint_D, freeze=args.freeze)
    ...
    train_log,val_log,test_log = train(model, optimizer, scheduler, train_dataloader, val_dataloader, use_mixup=args.mixup, mode=args.mode, epochs=args.epochs, device=device, name=args.name,fusion_mode=args.fusion_mode, run_name=args.run_name, use_retrain=args.retrain,test_dataloader=test_dataloader)
